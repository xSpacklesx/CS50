...
bool lookup(string s)
{
    ...
    for (int i = 0; i <  WORDS; i++)
    {
        ...
        if (strcmp(s, dictionary.words[i].letters) == 0 && !dictionary.words[i].found)
        {
            for (int l = 0; l < strlen(dictionary.words[i].letters); l++)
            {
...
// please log out before you leave ffs
...
void scramble(void)
{
    char sgrid[DIMENSION][DIMENSION];
    for (int i = 0; i < DIMENSION; i++)
    {

        for (int j = 0; j < DIMENSION; j++)
        {
            sgrid[j][DIMENSION - i - 1] = grid[i][j];
